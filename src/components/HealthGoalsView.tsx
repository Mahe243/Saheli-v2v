    ...
    <div id="health-goals-and-wins" className="space-y-8">
          ...
          >
            ...
            >
              ...
              <div className="space-y-2">
                <span className="text-[10px] font-display font-bold uppercase tracking-wider text-pink-600">Milestone Achieved!</span>
                <h3 className="font-display font-bold text-xl text-text-dark">Shabash, Priya!</h3>
                <p className="text-xs text-text-muted leading-relaxed font-sans">
                  You successfully completed your goal:<br />
                  <span className="font-bold text-indigo-600">"{celebratedGoal}"</span>
                </p>
              </div>
      ...
      <div className="bg-gradient-to-r from-pink-500/10 to-indigo-500/10 p-6 md:p-8 rounded-[24px] border border-pink-100">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h2 className="text-xl md:text-2xl font-display font-bold text-text-dark">Health Goals & Wins</h2>
            <p className="text-text-muted text-sm mt-1 max-w-2xl leading-relaxed">
              Track daily and weekly habits. Completing goals boosts your immune response, balances sleep, and triggers 
              automated reward logs in your Saheli ledger.
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="bg-white p-6 md:p-8 rounded-[24px] shadow-[0_4px_25px_rgba(0,0,0,0.01)] border border-gray-100">
            ...
            <AnimatePresence>
              {isAdding && (
                ...
                >
                  <h4 className="font-display font-bold text-sm text-text-dark">Create Custom Health Goal</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-display font-bold uppercase tracking-wider text-text-muted">Goal Title</label>
                      <input
                        type="text"
                        value={title}
                        onChange={(e) => setTitle(e.target.value)}
                        placeholder="e.g., Take Afternoon Nap, Walk in sun..."
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-display font-bold uppercase tracking-wider text-text-muted">Category</label>
                      <select
                        value={category}
                        onChange={(e) => setCategory(e.target.value as HealthGoal["category"])}
                        className="w-full p-2.5 rounded-lg border border-gray-200 bg-white text-xs text-text-dark focus:outline-none focus:ring-1 focus:ring-indigo-500"
                      >
                        <option value="Exercise">Exercise / Vyayam</option>
                        <option value="Water">Water / Hydration</option>
                        <option value="Medicine">Medicine / Supplement</option>
                        <option value="Stress">Stress Release / Dhyana</option>
                        <option value="Nutrition">Nutrition / Diet</option>
                      </select>
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-display font-bold uppercase tracking-wider text-text-muted">Target Value</label>
                      <input
                        type="number"
                        value={targetValue}
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-display font-bold uppercase tracking-wider text-text-muted">Unit</label>
                      <input
                        type="text"
                        value={unit}
                        onChange={(e) => setUnit(e.target.value)}
                        placeholder="minutes, glasses, days..."
                        className="w-full p-2.5 rounded-lg border border-gray-200 bg-white text-xs text-text-dark focus:outline-none focus:ring-1 focus:ring-indigo-500"
                      />
                    </div>
                  </div>
                ...
                return (
                  <div key={goal.id} className="p-5 rounded-2xl border border-gray-100 hover:shadow-2xs transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="space-y-2 flex-1">
                      ...
                      <h4 className="font-display font-bold text-sm text-text-dark">{goal.title}</h4>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-[24px] shadow-[0_4px_25px_rgba(0,0,0,0.01)] border border-gray-100 space-y-4">
            ...
            <p className="text-[10px] text-text-muted leading-relaxed">
              Every healthy choice Priya makes is logged here. Take pride in your wellness milestones!
            </p>
                ...
                achievements.map((ach) => (
                  <div key={ach.id} className="p-3.5 bg-pink-50/20 border border-pink-100/30 rounded-xl flex items-start gap-3 hover:bg-pink-50/40 transition-all">
                    ...
                    <div className="space-y-0.5">
                      ...
                      <h4 className="font-display font-bold text-xs text-text-dark leading-tight">{ach.title}</h4>
                      <p className="text-[10px] text-text-muted font-sans leading-normal">{ach.description}</p>
                    </div>
                  </div>
                ))
              ) : (
                <p className="text-xs text-text-muted text-center py-4 font-sans">No wins celebrated yet. Stay active!</p>
              )}
            </div>
          </div>
        </div>
